...
| Optional adapters | HTTP and MCP wrappers install through optional extras and route through shared dispatch. | Local Python 3.12 optional venv: `PYTHON=/private/tmp/jvm-memleak-py312-adapters/bin/python make test-adapters` passed 8 tests, including 6 HTTP adapter tests and 2 MCP wrapper tests. Hosted Python 3.11 optional adapter smoke passed in https://github.com/yaravind/jvm-memory-leak-debugger/actions/runs/26345458844. | Ready with hosted evidence |
| Cross-platform core | Linux, macOS, and Windows run manifest, parser, dispatch, install, CLI, and runtime-diagnostic smoke checks. | `Test JVM Memory Leak Debugger Skill` passed on Linux, macOS, and Windows for Python 3.9 and 3.12: https://github.com/yaravind/jvm-memory-leak-debugger/actions/runs/26345458844. | Ready with hosted evidence |
| MAT runtime | Linux, macOS, and Windows download, SHA-256 verify, unpack, and locate the pinned Eclipse MAT launcher without a heap dump. | `MAT Runtime Smoke` passed on Linux, macOS, and Windows with Java 17: https://github.com/yaravind/jvm-memory-leak-debugger/actions/runs/26345458850. | Ready with hosted evidence |
